...
public class WriteMemberListToPdfFile {
	public static void writeMemberListToPdfFile(String fileName, List<Users> memberList, HttpServletRequest request)
		...
		try {
			...
			PdfPCell cell1 = new PdfPCell(new Phrase("아이디", font)); // 셀의 이름과 폰트를 지정해서 셀을 생성한다.
			...
			for (int i = 0; i < memberList.size(); i++) {
				...
				PdfPCell cellNickname = new PdfPCell(new Phrase("" + users.getUser_name(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.

				PdfPCell cellReportCount = new PdfPCell(new Phrase("" + users.getBlame_count(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.

				PdfPCell cellActiveStatus = new PdfPCell(new Phrase("" + users.getEnabled(), font));
				// Phrase타입은 숫자형(int형 같은타입)으로 하면 에러가 발생되기 때문에 dto앞에 공백("")주어서 String타입으로 변경한다.

				table.addCell(cellUserId); // 셀의 데이터를 테이블에 저장한다. (장바구니안에 들어있는 갯수만큼 테이블이 만들어진다)
				table.addCell(cellUserEmail);
				table.addCell(cellNickname);
				table.addCell(cellReportCount);
				table.addCell(cellActiveStatus);
			}
